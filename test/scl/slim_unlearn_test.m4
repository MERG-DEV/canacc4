...
begin
  test_timeout: process is
    begin
      wait for 27740 ms;
      report("slim_unlearn_test: TIMEOUT");
      report(PC); -- Crashes simulator, MDB will report current source line
      PC <= 0;
      wait;
    end process test_timeout;
    --
  slim_unlearn_test: process is
    ...
    begin
      report("slim_unlearn_test: START");
      test_state := pass;
      ...
      report("slim_unlearn_test: Green LED (SLiM) on");
      --
      RB4 <= '0'; -- DOLEARN on
      RA5 <= '0'; -- UNLEARN on
      --
      ...
      if file_stat != open_ok then
        ...
        report("slim_unlearn_test: FAIL");
        PC <= 0;
        wait;
      end if;
      --
      report("slim_unlearn_test: Unlearn events");
      ...
      --
      report("slim_unlearn_test: Check events");
      while endfile(event_file) == false loop
        if RXB0CON.RXFUL != '0' then
          wait until RXB0CON.RXFUL == '0';
        end if;
      ...
      if test_state == pass then
        report("slim_unlearn_test: PASS");
      else
        report("slim_unlearn_test: FAIL");
      end if;          
      PC <= 0;
      wait;
    end process slim_unlearn_test;
end testbench;
