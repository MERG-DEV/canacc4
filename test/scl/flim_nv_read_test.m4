...
begin
  ...
  flim_nv_read_test: process is
    ...
    begin
      ...
      report("flim_nv_read_test: Yellow LED (FLiM) on");
      --
      ...
      --
      report("flim_nv_read_test: Ignore requests not addressed to node");
      ...
      --
      report("flim_nv_read_test: Read Node Variables");
      while endfile(data_file) == false loop
        wait for 1 ms; -- FIXME Next packet lost if previous Tx not yet completed
      ...
      report("flim_nv_read_test: Test beyond number of node variables");
      nv_index := nv_index + 1;
      RXB0D0 <= 16#71#;    -- NVRD, CBUS read node variable by index
      RXB0D1 <= 4;         -- NN high
      RXB0D2 <= 2;         -- NN low
      ...
      report("flim_nv_read_test: Test read node variable [0]");
      nv_index := nv_index + 1;
      RXB0D0 <= 16#71#;  -- NVRD, CBUS read node variable by index
      RXB0D1 <= 4;       -- NN high
      RXB0D2 <= 2;       -- NN low
